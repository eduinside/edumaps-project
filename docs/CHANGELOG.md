...
### 추가됨
- **학년별 로드맵 본문 링크**: 설명·탐구 질문·사후 활동에서 `[텍스트](URL)` 마크다운 링크와 맨 URL을 클릭 가능한 `<a>`로 렌더링. http/https만 허용해 XSS 차단, `dangerouslySetInnerHTML` 미사용. (`src/lib/richText.tsx` 신규)
- **백엔드 선택 영역 맞춤법 교정 메뉴(관리자용)**: 구글 시트 `📍 EduMaps 도구`에 '선택 영역 맞춤법 교정' 추가. 다음(Daum) 검사기로 선택 셀의 오탈자·띄어쓰기·맞춤법을 즉시 교정. API 키 불필요, 빈 셀·숫자·수식 제외, 한글 단어 경계 치환으로 과교정 방지, 서비스 오류 시 원문 보존. (`docs/backend/code.gs`)
